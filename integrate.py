...
def generate_traj_RK(ppp=500, periods = 40, itraj=0, path = ".", seed=0):
    ...
    os.makedirs(path+"{}/RK/".format(itraj), exist_ok=True)
    np.save(path+"{}/RK/states".format(itraj),states )
    np.save(path+"{}/RK/covs".format(itraj),covs )
    np.save(path+"{}/RK/signals".format(itraj),signals )
    np.save(path+"{}/RK/D".format(itraj),D)
    np.save(path+"{}/RK/C".format(itraj),C)
    np.save(path+"{}/RK/dt".format(itraj),np.array([dt]))
    np.save(path+"{}/RK/A".format(itraj),A)
